...
You should have a &lt;https://localhost:3000&gt; up and running if you have
followed the steps up until now but incase you don't have
&lt;https://locahost:3000&gt; running in you browser automatically. Please
attempt to open your browser and run the localhost url. If you have a simple
react interface - Good, but if not, try to go through the steps again and
carefully follow through till you have the desired result if not contact me :).
...
<blockquote>
  Installing the specific version of `ts-jest` and jest is important because the
  `ts-jest` team will throw a warning telling you that they have not tested the
  version you are using with the latest versions of `jest`. It also throws a
  version warning for typescript as well.
</blockquote>
...
<blockquote>
  Before you run the test remove `ts-jest` related configs from the
  `jest.config.js`. You can do that using the image below to see how i commented
  out `ts-jest` related changes.
</blockquote>
...
<figcaption>showing the git diff of the jest.config.js</figcaption>
...
<figcaption>showing the git diff of the package.json</figcaption>
