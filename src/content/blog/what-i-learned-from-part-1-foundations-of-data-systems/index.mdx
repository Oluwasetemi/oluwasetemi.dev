...
```typescript title="scalability-patterns.ts"
...
```typescript title="maintainability-patterns.ts"
...
```typescript title="normalization-decision.ts"
// When to normalize?
...
```typescript title="append-only-log.ts"
const actions = [
...
```typescript title="hash-index.ts"
...
```typescript title="sstables-indexeddb.ts"
...
```typescript title="code-splitting.ts"
...
```typescript title="sequential-writes.ts"
// SLOW: Random writes
...
```typescript title="network-reality.ts"
...
```typescript title="message-passing.ts"
...
```typescript showLineNumbers title="api-evolution.ts" wrap="false" ins={11} {21}
// API v1
interface User {
  id: string;
  name: string;
...
```typescript showLineNumbers title="append-only-log.ts" wrap="false"
...
```typescript collapse={3-4} title="compatibility-pattern.ts" wrap="false"
