...
# Clones and automatically cd into 'react' directory

# Clone with custom directory name
$ clone https://github.com/facebook/react my-react-fork
# Clones and automatically cd into 'my-react-fork' directory
```
...
```bash title="clone-v2.sh" del={2} ins={3-7}
function clone() {
...
```bash title="Terminal"
# Now supports shorthand notation!
$ clone facebook/react
# Expands to https://github.com/facebook/react.git

# Still works with full URLs
$ clone https://github.com/facebook/react
$ clone git@github.com:facebook/react.git

# With custom directory
$ clone facebook/react my-react-fork
```
...
```bash title="clone-v3.sh" ins={7-14} {8,10}
function clone() {
  # ... URL parsing logic ...
...
```bash title="Terminal"
# First clone - works as expected
$ clone facebook/react
# Clones into 'react' directory

# Try to clone again - automatic fallback!
$ clone facebook/react
# Initial clone failed, retrying with alternate folder name...
# Clones into 'react_facebook' directory

# No manual intervention needed
$ ls
react/  react_facebook/
```
...
```bash title="clone-v4.sh" ins={10-22} {11,13,15-16}
function clone() {
...
```bash title="Terminal"
# Clone the same repo multiple times - sequential numbering
$ clone facebook/react
# Clones into 'react'

$ clone facebook/react
# Clones into 'react_facebook'

$ clone facebook/react
# Clones into 'react_facebook_1'

$ clone facebook/react
# Clones into 'react_facebook_2'

# Can continue up to react_facebook_100
$ ls
react/  react_facebook/  react_facebook_1/  react_facebook_2/
```
...
```bash title="clone-v5.sh" ins={3-8,12-15} {4,6,13-14}
function clone() {
...
```bash title="Terminal"
# Copy URL from GitHub's branch view
$ clone https://github.com/facebook/react/tree/experimental
# Clones the repo AND checks out the 'experimental' branch
# You're immediately on the branch you wanted
...
# Also works with shorthand + manual branch checkout
$ clone facebook/react
...
```bash title="Terminal"
# All URL formats now work seamlessly
$ clone https://github.com/facebook/react/tree/experimental
DEBUG: Input URL: 'https://github.com/facebook/react/tree/experimental'
DEBUG: Extracted GitHub User: 'facebook'
DEBUG: Extracted Repo Name: 'react'
DEBUG: Branch: 'experimental'

$ clone git@github.com:facebook/react.git
DEBUG: Input URL: 'git@github.com:facebook/react.git'
DEBUG: Extracted GitHub User: 'facebook'
DEBUG: Extracted Repo Name: 'react'

$ clone facebook/react
DEBUG: Input URL: 'facebook/react'
DEBUG: Extracted GitHub User: 'facebook'
DEBUG: Extracted Repo Name: 'react'
```
...
<details name="update">
<summary><strong>November 5, 2025</strong> - Enhanced code examples and visual presentation</summary>

### What Changed:

- **Added Expressive Code frames**: All code blocks now display with beautiful syntax-highlighted frames featuring file name tabs (e.g., `clone-v2.sh`, `clone-v3.sh`)
- **Visual diff annotations**: Code blocks now show insertions (`ins={}`) and deletions (`del={}`) to clearly illustrate what changed between versions
- **Terminal usage examples**: Added practical terminal examples for each version showing real-world usage with sample commands and output
- **Syntax highlighting improvements**: Migrated to `astro-expressive-code` for better code presentation with dual themes (Dracula for dark mode, GitHub Light for light mode)

### Why These Changes:

These enhancements make the article more educational and easier to follow. The visual diffs help readers immediately see what changed between versions, while the terminal examples demonstrate practical usage patterns. The improved code presentation makes the evolution of the function clearer and more engaging.
